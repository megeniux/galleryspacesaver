...
class MediaPermissionStatus {
  ...
  bool get canScan => state == MediaPermissionState.granted || state == MediaPermissionState.partial;
...
class MediaRow {
  ...
  factory MediaRow.fromMap(Map<Object?, Object?> map) {
    ...
    return MediaRow(
      ...
      modifiedAt: DateTime.fromMillisecondsSinceEpoch(intValue(map['dateMillis']) ?? 0),
...
class MediaPlatform {
  ...
  Future<MediaPermissionStatus> permissionStatus() async {
    final result = await _channel.invokeMapMethod<String, Object?>('permissionStatus');
  ...
  Future<MediaPermissionStatus> requestPermissions({Iterable<MediaKind>? kinds}) async {
  ...
  Future<List<MediaRow>> scan({DateTime? modifiedAfter}) async {
    final result = await _channel.invokeListMethod<Object?>(
      ...
      <String, Object>{
        ...
        if (modifiedAfter != null) 'modifiedAfter': modifiedAfter.millisecondsSinceEpoch,
      },
    );
  ...
  Future<Uint8List?> thumbnail(String uri, MediaKind type) {
    return _channel.invokeMethod<Uint8List>(
      'thumbnail',
      <String, Object>{'uri': uri, 'type': type.name},
    );
  }
  ...
  Future<String> copyToCache({required String source, required String targetPath}) async {
    final result = await _channel.invokeMethod<String>(
      'copyMediaToCache',
      <String, Object>{'source': source, 'targetPath': targetPath},
    );
    if (result == null || result.isEmpty) throw StateError('Media could not be staged in app cache.');
    return result;
  }
  ...
  Future<String> compressImage({
    ...
    if (result == null || result.isEmpty) throw StateError('Image compression produced no output.');
    return result;
  }

  Future<Map<String, Object?>> processingGuards() async {
    return (await _channel.invokeMapMethod<String, Object?>('processingGuards')) ?? const <String, Object?>{};
  }

  Future<StorageInfo> storageInfo() async {
    final result = await _channel.invokeMapMethod<String, Object?>('storageInfo');
    return StorageInfo(
  ...
  Future<void> startProcessingService() => _channel.invokeMethod<void>('startProcessingService');

  Future<void> stopProcessingService() => _channel.invokeMethod<void>('stopProcessingService');

  Future<bool> requestMediaStoreConsent({required String uri, required String action}) async {
    return await _channel.invokeMethod<bool>('requestMediaStoreConsent', <String, Object>{'uri': uri, 'action': action}) ?? false;
  }

  Future<void> writeCachedFileToUri({required String sourcePath, required String uri}) {
    return _channel.invokeMethod<void>('writeCachedFileToUri', <String, Object>{'sourcePath': sourcePath, 'uri': uri});
  }

  Future<void> deleteMediaUri(String uri) {
    return _channel.invokeMethod<void>('deleteMediaUri', <String, Object>{'uri': uri});
  }

  Future<void> rescanMedia(String path) {
    return _channel.invokeMethod<void>('rescanMedia', <String, Object>{'path': path});
  }
  ...
  MediaPermissionStatus _statusFrom(Map<String, Object?>? result) {
    ...
    final granted = (result?['grantedTypes'] as List?)
            ?.whereType<String>()
            .map(
              (name) => MediaKind.values.where((kind) => kind.name == name).firstOrNull,
            )
            .whereType<MediaKind>()
            .toSet() ??
        const <MediaKind>{};
