...
class QueueState {
  const QueueState({
    this.status = QueueRunStatus.idle,
    this.notice,
    this.startedAt,
  });
...
class QueueController extends StateNotifier<QueueState> {
  ...
  /// Makes completed jobs retryable when their temporary outputs were cleared.
  /// The input media is left untouched; only stale output metadata is reset.
  Future<int> reconcileMissingOutputs() async {
    var requeued = 0;
    for (final job in await _database.getJobs()) {
      final outputPath = job.outputPath;
      if (job.status != 'done' || outputPath == null) continue;
      if (await File(outputPath).exists()) continue;
  ...
  Future<int> clearQueue() async {
    if (state.status != QueueRunStatus.idle) {
      throw StateError(
        'Pause or cancel the active batch before clearing the queue.',
      );
    }
    ...
    if (jobs.any(
      ...
      throw StateError(
        'Wait for the active file to finish before clearing the queue.',
      );
    }
    await CacheManagementService().clearCompressionCache();
    return _database.deleteAllJobs();
  }
  ...
  Future<void> start() async {
    ...
    final sessionId = await _database.insertSavingsSession(
      SavingsSessionsCompanion.insert(startedAt: startedAt),
    );
  ...
  void pause() {
    ...
    state = state.copyWith(
      status: QueueRunStatus.paused,
      notice: 'Paused after the current file finishes.',
    );
  }
  ...
  Future<void> _recordSession(int sessionId, DateTime startedAt) async {
    ...
    final finished = jobs
        ...
        .toList();
    final bytesFreed = finished.fold<int>(
      0,
      (sum, job) =>
          sum +
          ((job.outputSize ?? 0) < job.originalSize
              ? job.originalSize - (job.outputSize ?? 0)
              : 0),
    );
    await _database.updateSavingsSession(
      ...
      SavingsSessionsCompanion(
        ...
        skippedCount: Value(
          finished.where((job) => job.status == 'skipped').length,
        ),
        failedCount: Value(
          finished.where((job) => job.status == 'failed').length,
        ),
      ),
    );
  }
    ...
    if (attempt < 3 && code != FailureCode.cancelled) {
      await _database.updateJob(
        ...
        CompressionJobsCompanion(
          ...
          errorMessage: Value(result.reason),
          nextAttemptAt: Value(
            DateTime.now().add(Duration(seconds: 1 << (attempt - 1))),
          ),
          updatedAt: Value(DateTime.now()),
        ),
      );
  ...
  Future<MediaItem> _itemFromJob(CompressionJob job) async {
    final cached = job.mediaItemId == null
        ? null
        : await _database.getMediaItem(job.mediaItemId!);
  ...
  CompressionSettings _settingsFromJson(String value) {
    ...
    return CompressionSettings(
      preset: CompressionPreset.values.byName(
        json['preset'] as String? ?? CompressionPreset.balanced.name,
      ),
